...
第一步是计算 Query，Key，Value 的矩阵。首先，我们把所有词向量放到一个矩阵 X 中，然后分别和3 个权重矩阵$W^Q, W^K W^V$​​ 相乘，得到 Q，K，V 矩阵。（**X是2个单词，每个单词由4个embedding组成**）
...
Encoder-Decoder Attention层的原理和多头注意力（multiheaded Self Attention）机制类似，不同之处是：Encoder-Decoder Attention层是**使用前一层的输出来构造 Query 矩阵**，而 Key 矩阵和 Value 矩阵来自于解码器最终的输出。
